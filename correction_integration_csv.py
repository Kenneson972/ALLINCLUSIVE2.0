#!/usr/bin/env python3
"""
Script de correction - Mise à jour des 22 villas existantes avec données CSV
Le CSV est une mise à jour des villas existantes, pas l'ajout de nouvelles villas
"""
...
# Mapping correct : Villas existantes → Données CSV correspondantes
MAPPING_VILLA_CSV = {
    ...
    "Villa F5 Ste Anne": {
        ...
        "enhanced_description": "Villa F5 avec 4 chambres et 4 salles de bain. Possibilité d'accueillir jusqu'à 15 invités de 9h à 19h. Caution: 500€ en espèces + 1500€ en empreinte CB.",
    ...
    "Villa F3 POUR LA BACCHA": {
        ...
        "enhanced_description": "Villa F3 avec 2 chambres climatisées et salon climatisé avec canapé-lit. Possibilité d'accueillir jusqu'à 9 invités entre 9h et 18h. Caution: 1500€ par chèque.",
    ...
    "Studio Cocooning Lamentin": {
        ...
        "enhanced_description": "Studio cocooning avec bac à punch privé (petite piscine). Pas d'invités autorisés. Location uniquement à la semaine pendant les vacances scolaires.",
    ...
    "Villa François Moderne": {
        ...
        "enhanced_description": "Villa F3 sur les hauteurs du Morne Carrière. Stationnement pour 5 véhicules, enceintes JBL autorisées. Caution: 1000€. Check-in: 16h, Check-out: 11h.",
    ...
    "Villa Grand Luxe Pointe du Bout": {
        ...
        "enhanced_description": "Villa F6 avec piscine et jacuzzi. Fêtes autorisées de 10h à 19h. Check-in: 15h, check-out: 18h. Caution: 1000€ (empreinte bancaire).",
...
async def update_existing_villa_with_csv(db, villa_name, csv_data):
    """Met à jour une villa existante avec les données CSV"""
    try:
        ...
        # Préparer les données de mise à jour (on garde les données existantes et on ajoute les nouvelles)
        update_data = {
...
async def remove_duplicate_villas(db):
    """Supprime les villas créées en double par erreur"""
    try:
        # Supprimer les villas créées par erreur (qui ne sont pas dans les 22 originales)
        villas_to_remove = [
        ...
        deleted_count = 0
        for villa_name in villas_to_remove:
            result = await db.villas.delete_one({'name': villa_name})
            if result.deleted_count > 0:
                deleted_count += 1
                print(f"🗑️  Villa supprimée: {villa_name}")
        
        print(f"📊 Villas supprimées: {deleted_count}")
        return deleted_count
        
    except Exception as e:
        print(f"❌ Erreur suppression villas: {e}")
        return 0

async def main():
    """Fonction principale de correction"""
    print("🔧 CORRECTION INTÉGRATION CSV - MISE À JOUR DES 22 VILLAS EXISTANTES")
    print("=" * 70)
    ...
    try:
        ...
        print(f"📊 Villas actuelles en base: {current_count}")
        
        # Supprimer les villas créées par erreur
        print("\n🗑️  SUPPRESSION DES VILLAS CRÉÉES PAR ERREUR")
        print("-" * 45)
        await remove_duplicate_villas(db)
        
        # Vérifier le nouveau nombre
        new_count = await db.villas.count_documents({})
        print(f"📊 Villas après nettoyage: {new_count}")
        
        # Mettre à jour les 22 villas existantes avec les données CSV
        print("\n🔄 MISE À JOUR DES 22 VILLAS EXISTANTES")
        print("-" * 40)
        
        updated_count = 0
        total_mapped = len(MAPPING_VILLA_CSV)
        
        for villa_name, csv_data in MAPPING_VILLA_CSV.items():
            success = await update_existing_villa_with_csv(db, villa_name, csv_data)
            if success:
                updated_count += 1
        
        print(f"\n📊 Villas mises à jour: {updated_count}/{total_mapped}")
        
        # Vérifier le résultat final
        final_count = await db.villas.count_documents({})
        updated_villas = await db.villas.count_documents({'csv_updated': True})
        
        print(f"📈 Villas totales: {final_count}")
        print(f"📋 Villas avec données CSV: {updated_villas}")
        
        # Afficher quelques exemples
        sample_villas = await db.villas.find({'csv_updated': True}).limit(3).to_list(3)
        ...
        report = {
            ...
            "total_villas_final": final_count,
            "expected_villas": 22,
            "updated_from_csv": updated_count,
            "correction_date": datetime.utcnow(),
            "interface_preserved": True,
            "approach": "mise_a_jour_existantes"
        }
        
        # Sauvegarder le rapport
        await db.correction_reports.insert_one(report)
        
        print("\n✅ CORRECTION TERMINÉE")
        print("🎯 Interface conservée, 22 villas mises à jour avec données CSV")
        print("💰 Système de tarification variable activé")
        print("📊 Rapport de correction sauvegardé")
        
        if final_count == 22:
            print("\n🎉 SUCCÈS: Exactement 22 villas comme prévu!")
        else:
            print(f"\n⚠️  ATTENTION: {final_count} villas au lieu de 22")
